...
public sealed record Inspection(Guid Id, Guid CompanyId, Guid PropertyId, string Code, string PropertyName, string Type, InspectionStatus Status, DateTime ScheduledAt, string Inspector, int Completion, int PendingItems, string? CoverUrl);
...
public sealed record Occurrence(Guid Id, Guid CompanyId, Guid InspectionId, string Title, string Priority, string Status, DateTime DueDate, decimal EstimatedCost, string PropertyName);
