...
async function showInspection(id) {
  ...
  const roomsHtml = rooms.map(room => { const list = items.filter(i => i.room === room);
    ...
    return `<div class="room-group"><div class="room-head"><h4>▤ ${esc(room)}</h4><span class="room-count">${done}/${list.length} avaliados</span></div>${list.map(c => itemHtml(c, ++n)).join('')}<div class="add-topic"><input class="new-topic-input" placeholder="Adicionar tópico em ${esc(room)}"><button type="button" class="ghost" data-add-item="${esc(room)}">+ Tópico</button></div></div>`;
  ...
  $('#pageContent').innerHTML = `<form id="fieldInspectionForm" class="inspection-form" data-id="${id}">
    ...
    <div class="inspection-layout">
      <div class="inspection-main">
        ...
        <section class="form-card final-card"><div><span class="eyebrow" style="color:var(--gold-2)">CONCLUSÃO</span><h3>Fechar vistoria</h3><p>Revise os itens de atenção antes de enviar para revisão.</p></div><button type="button" class="primary" id="completeBtn">Concluir vistoria <span>→</span></button></section>
      </div>
...
function signOut() { token.clear(); currentUser = null; $('#appShell').classList.add('hidden'); $('#loginScreen').classList.remove('hidden'); document.body.classList.remove('nav-open'); }
...
document.addEventListener('click', async e => {
  ...
  if (t.id === 'completeBtn') { const f = $('#fieldInspectionForm'); try { await api(`inspections/${f.dataset.id}/complete`, { method:'POST' }); await loadData(); toast('Vistoria enviada para revisão.','ok'); renderView('inspections'); } catch(err){ toast(err.message,'warn'); } return; }
...
['modal','propertyModal','simpleModal','templateModal'].forEach(id => $('#'+id).addEventListener('click', e => { if (e.target.id === id) closeModal(id); }));
